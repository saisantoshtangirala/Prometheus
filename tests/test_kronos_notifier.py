"""
Tests for kronos/notifier.py - the daily Telegram progress digest.
...
from kronos.notifier import MAX_MESSAGE_CHARS, TelegramNotifier
...
class TestEnabledGating:
    def test_disabled_without_env_vars(self, config, monkeypatch):
        monkeypatch.delenv("KRONOS_TELEGRAM_BOT_TOKEN", raising=False)
        monkeypatch.delenv("KRONOS_TELEGRAM_CHAT_ID", raising=False)
        notifier = TelegramNotifier(config)
        assert not notifier.enabled

    def test_disabled_with_only_token_set(self, config, monkeypatch):
